...
while getopts ":c:a:d:i:z:l:g:e:r:s:j:p:" opt;
do  case "$opt" in
    c) CORE_HOME=$OPTARG;;
    a) APPS_HOME=$OPTARG;;
    d) S4_CLOCK=$OPTARG;;
...
if [ "x$PORT" == "x" ] ; then
    echo "No s4 connection port specified"
    exit 1
fi
...
CMD="${JAVA_LOC}java $GC_OPTS $DEBUG_OPTS $MEM_OPTS $JAVA_OPTS -classpath $CORE_HOME$CP_SEP$CLASSPATH -DDequeuerCount=6 -Dlog4j.configuration=file:${CONF_LOC}/log4j.xml io.s4.MainApp -c ${CORE_HOME} -a ${APPS_HOME} -e ${EXTS_HOME} -t ${CONF_TYPE} -d ${S4_CLOCK} -p ${PORT} $S4_OPTS"
echo "RUNNING $CMD"
