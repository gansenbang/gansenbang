...
echo "Usage: $0" >&2
  ...
  echo " -p adapter connection port" >&2
  echo " -h help" >&2
  exit 1
fi
...
while getopts ":c:z:a:s:g:l:u:d:p:x" opt;
do case "$opt" in
    c) CORE_HOME=$OPTARG;;
    a) APPS_HOME=$OPTARG;;
...
if [ "x$PORT" == "x" ] ; then
    echo "No adapter connection port specified"
    exit 1
fi
...
CMD="${JAVA_LOC}java $JAVA_OPTS -Dlog4j.configuration=file:${CONF_LOC}/log4j.xml -classpath $CLASSPATH io.s4.client.Adapter -t ${CONF_TYPE} -c ${CORE_HOME} -d ${DATA_ADAPTER_CONF} -p ${PORT}"
echo "Running ${CMD}"
exec $CMD
