...
public class Dispatcher implements EventDispatcher {
		...
		Runnable r = new Runnable() {
			...
			public void run() {
				...
				while (!Thread.currentThread().isInterrupted()) {
					...
					Logger.getLogger(loggerName).info(
							"Event count is " + eventCount + "; rate " + rate);
					Logger.getLogger(loggerName).info(
							"Raw event count is " + rawEventCount + "; rate "
									+ rawRate);
					if (counts != null) {
			...
			private void configCheck() {
				...
				if (lastModified > configFileTime) {
					Logger.getLogger(loggerName).info(
							"Config file has changed. Exiting!!");
					System.exit(4);
				}
			}
		};
	...
	private void dispatchEvent(String streamName, Object event,
			boolean variableKey, List<List<String>> compoundKeyNames) {
		synchronized (this) {
			rawEventCount++;
		}
		if (eventEmitter.getNodeCount() <= 0) {
			...
			if (counts == null || oldnodes != eventEmitter.getNodeCount()) {
				counts = new int[eventEmitter.getNodeCount()];
				oldnodes = eventEmitter.getNodeCount();
			}
		}
		...
		try {
			...
			Logger.getLogger(loggerName).error(
					"Exception in processEvent on thread "
