...
public class ClientConnection {
	...
	public void close() {
		...
		try {
			...
			ClientStub.logger.error(
					"problem closing client connection to client " + uuid, e);
		}
	}
	...
	public final Runnable receiver = new Runnable() {
		public void run() {
			try {
				while (good()) {
					...
					EventWrapper ew = ClientConnection.this.clientStub
							.eventWrapperFromBytes(b);
					if (ew == null)
						continue;
		...
		private void decorateRequest(Request r) {
			...
			if (info != null && info instanceof Request.ClientRInfo)
				((Request.ClientRInfo) info)
						.setRequesterUUID(ClientConnection.this.uuid);
		}
	};
