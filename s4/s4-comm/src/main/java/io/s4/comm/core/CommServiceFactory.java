...
public class CommServiceFactory {
	...
	public static TaskManager getTaskManager(String zkaddress,
		...
		if (mode != null && mode.equalsIgnoreCase("static")) {
			...
			taskManager = new StaticTaskManager(zkaddress, clusterName,
					clusterType, callbackHandler);
		} else {
			taskManager = new ZkTaskManager(zkaddress, clusterName,
					clusterType, callbackHandler);
		}
	...
	public static ProcessMonitor getProcessMonitor(String zkaddress,
		...
		if (mode != null && mode.equalsIgnoreCase("static")) {
			logger.info("Comm layer mode is set to static");
			processMonitor = new StaticProcessMonitor(zkaddress, clusterName,
					ClusterType.S4);
		} else {
			processMonitor = new ZkProcessMonitor(zkaddress, clusterName,
					ClusterType.S4, callbackHandler);
		}
